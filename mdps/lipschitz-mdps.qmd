...
_Wasserstein distance_ between distributions $\mu$ and $\nu$ is defined as:[^definition]
$$ K(\mu,\nu) = \sup_{f : \| f\|_L \le 1 }
   \left| \int_{\ALPHABET X} f\, d\mu - \int_{\ALPHABET X} f\, d\nu \right|. $$

[^definition]: The definition provided here is actually the dual of the standard definition of Wasserstein distance. See @thm-Kantorovich for details.

The next results follow immediately from the definition of Wasserstein
distance.
...
The Wasserstein distance is a special class metrics on probability spaces known as [integral probability meterics (IPMs)][IPM]. @prp-Kantorovich is a special case of a similar general result for IPMs (@prp-IPM-ineq).
