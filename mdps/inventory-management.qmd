...
::: {.column-margin}
![Image credit: ChatGPT-4o][inventory]
...
T = 15
xmin, xmax = -50, 50
ojs_define(T=T, xmin=xmin, xmax=xmax)
...
Define 
$$ σ_t = \arg \min_{z \in \reals} \bigl\{ p z + H_t(z) \bigr\} . $$
Then, 
\begin{equation} \label{eq:V}
V^*_t(s) = \begin{cases}
  H_t(σ_t) + p (σ_t - s), &\text{if } s \le σ_t \\
  H_t(s)   , & \text{otherwise }
\end{cases} 
\end{equation}
and
...
policy. It states that there is a base-stock level $\{σ_t\}_{t \ge 1}$
for every time step. If, at the beginning of time $t$, the value of the
current stock is below the base stock level $σ_t$, then the optimal
...
2. For any convex function $f \colon \reals \to \reals$, 
   let $σ =  \arg \min_{s \in \reals} f(s)$. (If the arg min is not unique, take $σ_t$ to be the _smallest_ arg min). Then,
...
If there were no constraint on $a$, then the minimizer will be $a = σ -
s$. If $s \le σ$, then $a = σ -s \in \reals_{\ge 0}$ is the minimizer for
the constrained problem as well. On the other hand, if $s \ge σ$, then
...
$$H_T(z) = \EXP[ h(z - W) ] $$
which is convex because $h(z)$ is convex. $f_T(z) = p z + H_T(z)$ is the
sum of a linear function and convex function and is, therefore, convex. 
Then, by fact 2 above,
$$π^*_T(s) = \arg \min_{a \in \reals_{\ge 0}} f_T(s+a) = \max(σ_T - s, 0)
$$
and
$$V^*_T(s) = \min_{a \in \reals_{\ge 0}} f_T(s + a) - p s = 
  \begin{cases}
    f_T(s) - p s, & \text{if } s > σ_T \\
    f_T(σ_T) - p s, & \text{if } s \le σ_T.
  \end{cases}
$$
...
## Variations of a theme: lost sales {#lost-sales}

Consider a variation of the basic model where the excess demand is lost rather than backlogged. Thus, $\S = [0, ∞)$ and the stock evolves according to
$$
S_{t+1} = [S_t + A_t - W_t]^{+} 
$$
where $[x]^{+} \coloneqq \max\{x, 0\}$. There is still a negative per-step cost associated with excess demand. Thus, the per-step cost is
$$
p A_t + h(S_t + A_t - W_t).
$$

We will write the DP slightly differently (in terms of the function $f_t$ rather than $H_t$). Define
$$ V^*_{T+1}(s) = 0$$
and for $t \in \{T, \dots, 1\}$, 
$$
  f_t(z) = pz + \EXP[c_h [z-W]^{+} - c_s [z-W]^{-} + V^*_{t+1}([z - W]^{+})]
$$
where $[x]^{-} = \min\{0, x\}$
and
$$ \begin{align*}
  V^*_t(s) &= \min_{a \ge 0} \bigl\{  f_t(s+a) \bigr\} - ps , \\
  π^*_t(s) &= \arg \min_{a \ge 0} \bigl\{ f_t(s+a) \bigr\}.
\end{align*} $$


:::{#thm-inventory-lost-sales}
Assume that $p < c_s$. Define 
$$ σ_t = \arg \min_{z \in \reals} \bigl\{ f_t(z) \bigr\} . $$
Then,  $σ_t \ge 0$, 
\begin{equation} \label{eq:V-lost}
V^*_t(s) = \begin{cases}
  f_t(σ_t) - ps , &\text{if } s \le σ_t \\
  f_t(s) - p s   , & \text{otherwise }
\end{cases} 
\end{equation}
and
\begin{equation}\label{eq:pi-lost}
  π^*_t(s) = \begin{cases}
  σ_t - s, &\text{if } s \le σ_t \\
  0, & \text{otherwise }
\end{cases}\end{equation}
...
Note that we cannot follow the same proof argument as before because for a convex function $\EXP[f([s-W]^{+})]$ is not necessarily convex. So we cannot show that $H_t(z)$ is convex. So, we need a slightly different proof argument.

::: {.callout-note collapse="true"}
#### Proof 
We will show the following fact.

**Fact.** If $f_{t+1}$ is convex, then $V^*_{t+1}, π^*_{t+1}$ is of the specified form and 
$$g_t(z) = V^*_{t+1}([z]^{+}) - c_s [z]^{-}$$ 
is convex.

**Implication of fact.** Convexity of $g_t$ implies that
$g_t(z) + c_h [z]^{+}$ is convex (since it is sum of two convex functions). Hence, $f_t(z)$ is convex (since it is sum of a linear function and weighted average of convex functions). This implies the result holds by induction.

**Proof of fact.** The fact that convexity of $f_{t+1}$ implies the structure of $V^*_{t+1}$ and $π^*_{t+1}$ follows from the same argument as @thm-inventory-structure. Then, by definition of $g_t$, we have
$$
g_t(z) = \begin{cases}
f_{t+1}(σ_{t+1}) - c_s z, & \hbox{if } 0 < z \\
f_{t+1}(σ_{t+1}) - p z,   & \hbox{if } 0 \le z < σ_{t+1} \\
f_{t+1}(z) - p z,   & \hbox{if } σ_{t+1} \le z 
\end{cases}
$$
In the range $(-∞, 0)$ the function is linear and has slope $-c_s$; in the range $[0, σ_{t+1})$ the function is linear and has slope $-p$; in the range $[σ_{t+1}, ∞)$, the function is convex and increasing and has a slope greater than $-p$. Hence, the slope of $g_t$ is increasing from $(-∞,∞)$ and hence the function is convex.

The above argument also shows that the slope of $g_t(z)$ (and therefore $g_t(z) + c_h [z]^{+}$ is $-c_s$ in $(-∞,0)$. Hence, the slope of $f_t(z)$ is strictly negative in $(-∞, 0)$. Thus, $σ_t \not\in (-∞,0)$. 

:::

## Variations of a theme: positive ordering cost {#positive-ordering-cost}

We now consider the case when the store has pay a fixed cost $K$ every time it places an order. Thus, the cost of ordering the inventory is
$$
  \begin{cases}
  pa + K, & \hbox{if } a > 0 \\
  0, & \hbox{otherwise}
...
To understand the nature of the optimal solution, we repeat the simulation given above with $K=4$.
...
:::{#lem-s-S-property}
Let $f \colon \reals \to \reals$ be a continuous $K$-convex function such that $f(x) \to \infty$ as $\ABS{x} \to \infty$. Then, there exist scalars $σ$ and $Σ$ with $σ \le Σ$ such that 
...
Since $f$ is a continuous function and $f(x) \to ∞$ as $\ABS{x} \to ∞$, there exists a minimizing point of $f$, which we denote by $Σ$. Let $σ$ be defined as
$$
  σ = \inf\{ z \le Σ : f(z) = f(Σ) + K \}.
$$
Since $f(-∞) \to ∞$, such a $σ$ must exist.
...
3. Note that for any $x < y < σ$, we have (taking $u - b = x$, $u = y$, and $u + z = Σ$ in the second definition of $K$-convexity)
   $$
   K + f(Σ) \ge f(y) + \frac{Σ - y}{y - x}( f(y) - f(x) )
   $$
...
4. First observe that the property is true for $y = z$, or for $y = Σ$ or $y = σ$. So, there are two remaining possibilities: $y < Σ$ or $y > Σ$. 
      ...
      If $σ < y < Σ$, then by $K$-convexity we have
      $$
        f(σ) = f(Σ) + K \ge f(y) + \frac{Σ-y}{y-σ}(f(y) - f(σ)).
      $$
      Rearranging terms, we have
      $$
      \left( 1 + \frac{Σ - y}{y - σ}\right)f(σ) 
      \ge 
      \left( 1 + \frac{Σ - y}{y - σ}\right)f(y).
      $$
      Thus, $f(σ) \ge f(y)$. The result then follows by observing that 
      $$
      f(z) + K \ge f(Σ) + K = f(σ) \ge f(y).
      $$

      If $Σ < y < z$, then by $K$-convexity we have
      $$
        K + f(z) \ge f(y) + \frac{z - y}{y - Σ}(f(y) - f(Σ))
        \ge f(y),
      $$
...
Define 
$$
  Σ_t = \arg \min_{z \in \reals} \big\{
  pz + H_t(z) \bigr \}
$$
and 
$$
  σ_t = \inf \{ z \le Σ_t : pz + H_t(z) = pσ_t + H_t(Σ_t) + K \}.
$$

Then,
\begin{equation}\label{eq:value-setup}
  V^*_t(s) = \begin{cases}
  K + H_t(Σ_t) + p(Σ_t - s), & \hbox{if } s < σ_t \\
...
We now assume that $H_t(z)$ is $K$-convex. Note that @lem-K-convex implies (part 2) implies thta $f_t(z)$ is also $K$-convex. By definition, $Σ_t$ is the minimizer of $f_t(z)$ and $σ_t$ is the smallest value of $z$ for which $f_t(z) = f_t(Σ_t) + K$. Therefore,
\begin{equation}\label{eq:V-setup}
V^*_t(s) = 
\begin{cases}
  K + f_t(Σ_t) - ps, & \hbox{if } s < σ_t \\
...
- **Case 2: $s \le s + z \le σ_t$.**
  In this region, $V^*_t(s)$ is affine and therefore $K$-convex.

- **Case 3: $s < σ_t < s + z$.**
In this case, left hand side of \eqref{eq:verify-setup} equals
\begin{align*}
&K + f_t(s+z) - p(s+z) - [K + f_t(Σ_t) - ps] + pz 
\\ &\quad
=
K + f_t(s+z) - p(s+z) -[K + f_t(Σ_t) - p(s+z)]
\end{align*}
which is positive because action we know that at $s + z > σ_t$, not ordering is better than ordering.
...
2. Argue that the structure of optimal policies continue to hold, i.e., there exists a sequence $\{σ_t\}$, $s_t \in \integers$ such that the policy 
   $$
    π^*_t(s) = \begin{cases}
    σ_t - s, & \text{if $s \le σ_t$} \\
    0, & \text{otherwise}.
...
### Bounded inventory level

Consider inventory management with zero ordering cost with the only difference being that there is an upper bound $\overline B$ and a lower bound $\underline B$ on the allowable value of the stock. In particular, let $W_{\max} > 0$ be the maximum value of the demand (and we assume $\underline B + W_{\max} < \overline B$), then the bounds impose a constraint
$$
...
Inventory management models with deterministic demand were introduced by @Harris1913. The mathematical model of inventory management considered here was originally proposed by @Arrow1951. The optimality of base-stock policy was established by @Bellman1955. For a detailed overview of inventory management, see @Zipkin2000.
...
The following generalizations of the basic model are presented in these notes:

- See the notes on [inventory management with lost demand](#inventory-lost-demand) that the optimal policy is a base-stock policy even when excess demand is lost rather than backlogged (i.e., when the dynamics are of the form $S_{t+1} = \max\{S_t + A_t - W_t, 0\}$. 

- See the notes on [infinite horizon](inventory-management-revisited.html) version of this model to see how to find the threshold in closed form. 

Sufficient conditions for the optimality of $(s,S)$-policies were presented in @Dvoretzky1953. 
The notion of $K$-convexity and optimality of $(s,S)$-policies is due to @Scarf1960. An alternative proof under a different set of assumptions was provided by @Veinott1966. The proof of @lem-s-S-property is borrowed from @Bertsekas:book. The proof of @thm-s-S-policy is from @Scarf1960. See @Bertsekas:book for an alternative proof of @thm-s-S-policy.
