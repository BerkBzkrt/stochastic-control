...
Consider a _linear_ stochastic system with state $x_t \in \reals^{d_x}$ and controls $u_t \in \reals^{d_u}$ where the dynamics are given by
\begin{equation}\label{eq:dynamics}
  x_{t+1} = A x_t + B u_t + w_t,
  \quad t \ge 1
\end{equation}
where $A \in \reals^{d_x × d_x}$ and $B \in \reals^{d_x × d_u}$ are known matrices and $\{w_t\}_{t \ge 1}$, $w_t \in \reals^{d_x}$ is process noise. The objective is to choose the control policy $g = (g_1, \dots, g_{T-1})$, where $u_t = g_t(x_{1:t}, u_{1:t-1})$, to minimize
$$
...
Let $x \in \reals^{d_x}$ be a random variable with mean $μ$ and covariance $Σ$. Then,
$$
\EXP[ x^\TRANS P x ] = μ^\TRANS P μ + \TR(P Σ)
$$
:::
...
- $Δ = R + B^\TRANS P B$
- $K = Δ^{-1}[ S^\TRANS + B^\TRANS P_{+} A ]$
- $P = Q + A^\TRANS P_{+} A - K^\TRANS Δ K$
:::
...
Given the system model $(A,B)$ and per-step cost $(Q,S,R)$, we define the _Riccati operator_ $\RICCATI \colon \mathbb{S}^{d_x × d_x}_{\ge 0} \to \mathbb{S}^{d_x × d_x}_{\ge 0}$ as follows:
$$ \RICCATI P = Q + A^\TRANS P A 
- (S^\TRANS + B^\TRANS P A)^\TRANS (R + B^\TRANS P B)^{-1}
  (S^\TRANS + B^\TRANS P A).$$
Moreover, define the _Gain_ operator $\GAIN \colon \mathbb{S}^{d_x × d_x}_{\ge 0} \to \reals^{d_u × d_x}$ as
...
\begin{align*} 
  ...
  &
  B &= \int_{0}^{Δt} e^{A_ct} B_c dt = \MATRIX{\tfrac12 Δt^2 \\ \frac{Δt}{d_u}}
\end{align*}
...
Consider a system with two subsystems: subsytem 1 with state $x^1_t \in \reals^{d_x^1}$ and control $u^1_t \in \reals^{d_u^1}$ and subsystem 2 with state $x^2_t \in \reals^{d_x^2}$ and control $u^2_t \in \reals^{d_u^2}$. The dynamics are coupled only though the noise, i.e., 
\begin{align*}
