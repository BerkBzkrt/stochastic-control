...
:::{#exr-info-state-irrelevant}
#### MDP with irrelevant component

Consider an MDP with state space $(\ALPHABET S^1 × \ALPHABET S^2)$ and action space $\ALPHABET A$. Suppose the transition matrix satisfies
\begin{multline*}
  \PR(S^1_{t+1} = s^1_{t+1}, S^2_{t+1} = s^2_{t+1}
  \mid S^1_t = s^1_t, S^2_t = s^2_t, A_t = a_t)
  \\
  =
  \PR(S^1_{t+1} = s^1_{t+1}
  \mid S^1_t = s^1_t, A_t = a_t)
  \PR(S^2_{t+1} = s^2_{t+1}
  \mid S^1_t = s^1_t, S^2_t = s^2_t, A_t = a_t)
\end{multline*}
and the per-step costs does not depend on $s^2$, i.e., 
$$
c(s^1,s^2,a) = c(s^1,a).
$$
Then, $\{S^1_t\}_{t \ge 1}$ is an information state.
:::

:::{#exr-info-state-folded}
#### Folded MDP
...
:::{#exr-info-state-delayed}
#### MDP with delayed state observation

Consider an MDP where the agent observes the state $S_t$ with a delay $δ$, i.e., the observation $Y_t$ at time $t$ equals $S_{t-δ}$. Show that $(S_{t-δ}, A_{t-δ:t-1})$ is an information state.
:::

:::{#exr-info-state-delayed-observation}
#### POMDP with delayed observations

Consider an POMDP where the agent observes the observation $Y_t$ with a delay $δ$, i.e., the observation $Y_t$ at time $t$ equals $\ell_t(S_{t-δ}, N_{t-δ})$. Show that $(B_{t-δ}, A_{t-δ:t-1})$ is an information state.
:::
...
@exr-info-state-irrelevant is due to @Feinberg2005. As explained in @Feinberg2005, such models arise in control of queues and transformation of continuous time MDPs to discrete time MDPs using uniformization. It may also be considered an instance of what is called the _noisy TV problem_ in the RL literature [@Burda2019]. 

@exr-info-state-folded is due to @Chakravorty2018. @exr-info-state-delayed is due to @Altman1992. @exr-info-state-delayed-observation is due to @Bander1999. @exr-info-state-machine-repair is an instance of incrementally expanding representations for POMDPs presented in @Arabneydi2015.
